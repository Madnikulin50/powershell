...
.PARAMETER ExportToFile
    This is a SWITCH that, if specified, will store the results in a json file.
    This file will be placed on the directory where the script is located, and named :
        "GetEventsFromEventLogs_EventID1-EventID2-XXXXX_Year-MONTH-DAY-Hour-Minute-Second.json"
    Example:
        "GetEventsFromEventLogs_916-105_2018-04-13-09-52-08.json"
...
.\Get-EventsFromEventLogs.ps1 -Computers SRV-EX-01,SRV-EX-02,SRV-EX-03 -EventLevel Error -ExportToFile
    ...
    will be dumped into a file labelled GetEventsFromEventLogs-Date-time.json as we specified the
...
.\Get-EventsFromEventLogs.ps1 -NumberOfLastEventsToGet 10 -EventID 916,105 -ExportToFile
    ...
    The exported file will be named GetEventsFromEventLogs_916-105_2018-04-13-10-01-55.json
...
.\Get-EventsFromEventLogs.ps1 -EventSource "disk" -NumberOfLastEventsToGet 1000 -EventLevel Critical,Warning,Error -ExportToFile
    ...
    - Export into a file (like GetEventsFromEventLogs_None_2018-04-14-04-34-27.json)
...
Param(
    ...
    [Parameter(Mandatory = $False, Position = 8, ParameterSetName = "NormalRun")] [Boolean]$Confirm = $false,
...
1.4 -> 1.4.1
Fixed file name generation when not supplying any EventID and no EventSource -> putting "Last_X_" inside the json name
1.3 -> 1.4
...
# $EventsReport = "$PSScriptRoot\GetEventsFromEventLogs_$(get-date -f yyyy-MM-dd-hh-mm-ss).json"
...
If ($ExportToFile){
    If (!(IsEmpty $EventID)){
        $FileEventLogFirstID = "GetEventsFromEventLogs_$($EventID[0])_$(get-date -f yyyy-MM-dd-hh-mm-ss).json"
        If (IsPSV3){
        ...
        If (!(IsEmpty $EventSource)){
            $FileEventLogFirstSource = "GetEventsFromEventLogs_$($EventSource[0])_$(get-date -f yyyy-MM-dd-hh-mm-ss).json"
            If (IsPSV3){
            ...
            $FileNumberOfLastEvents = "GetEventsFromEventLogs_Last_$($NumberOfLastEventsToGet)_$(get-date -f yyyy-MM-dd-hh-mm-ss).json"
            If(IsPSV3){
    ...
    $Events4all |  ConvertTo-Json | Out-File -FilePath $EventsReport -Encoding UTF8 -Append 
